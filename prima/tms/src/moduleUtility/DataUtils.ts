...
export function setAutoCompleteListFromObject(params: Array<any> | undefined, parentKey: string, labelKey?: string, valueKey?: string): Array<OptionType> {
    // use element  as label and value when array type is string
    return (params && params.map((element: any) => ({
...
export function setAutoCompleteListWithData(params: Array<any> | undefined, labelKey?: string, valueKey?: string): Array<OptionType> {
    // use element  as label and value when array type is string
    return (params && params.map((element: any) => ({
        label: (labelKey && element[labelKey]) || "label",
        value: (valueKey && element[valueKey]) || "value",
...
export function setAutoCompleteListCustomLabel(params: Array<any> | undefined, labelKey: string, valueKey: string, helperKey: string): Array<OptionType> {
    // use element  as label and value when array type is string
    return (params && params.map((element: any) => ({
        label: ((labelKey && element[labelKey]) + ((helperKey && element[helperKey]) ? (" ( " + element[helperKey] + " )") : "")) || "label",
        value: (valueKey && element[valueKey]) || "value",
        data: element,
    }))) || [];
}
