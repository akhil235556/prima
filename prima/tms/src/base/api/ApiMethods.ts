    ...
    (error: any) => {
        ...
        if (error.response && error.response.data) {
            ...
            if (error.response.status === 413) {
                return Promise.reject((typeof error.response.data) === 'string' ? { message: error.response.data, status: error.response.status } : { ...error.response.data, status: error.response.status });
            } else {
