...
import { articleError, errorFreightType, errorLane, errorSelectTransporter, quantityError, vehicleTypeError } from '../../base/constant/MessageUtils';
...
interface DiversionRenderArticlesProps {
    element: any,
    onAdd: any,
    onRemove: any,
    onSelectArticle: any,
...
export function DiversionRenderArticles(props: DiversionRenderArticlesProps) {
    const appDispatch = useDispatch();
    const { onAdd, onRemove, element, onSelectArticle, onChangeArticle, classStyle, DODetails, doMaterialList, onChangeQuantity, onChangeDOMaterial, onChangeProductQuantity, onChangeUom, onClear, order, showDONumber } = props;
                ...
                <div className="custom-form-row row align-items-end">
                    <div className="col-md-12 ">
                        <div className="row align-items-center">
                            {((order && order !== "DO") || !showDONumber) && isNullValue(DODetails) ?
                                <>
                                    ...
                                    <div className="form-group col-md">
                                        <NumberEditText
                                            ...
                                            // disabled={isObjectEmpty(element.article) || element.isDisabledMaterialQuantity}
                                ...
                                <>
                                    <div className="form-group col-md-4">
                                        <AutoComplete
                                            ...
                                            mandatory
                                            isDisabled={isNullValue(DODetails)}
...
export function validateIndentData(userParams: any, laneCheck: any, laneVisible: boolean): any {
    if (userParams.freightType === undefined) {
        ...
        return { serviceabilityModeCode: "Enter valid mode of transportation" }
    }
...
export function diversionValidateShipmentDetails(userParams: any, freightType?: any, doResponse?: any, order?: any, showDONumber?: boolean): any {

    const shipmentDeatils = userParams.map((element: any) => {
        if ((order && order === "DO") || showDONumber) {
            if (isNullValue(element.details.demandOrderCode)) {
                element.error.demandOrderCode = "Enter DO Number"
            }
        }
    ...
    const error = shipmentDeatils.some(function (element: any) {
        return ((
            !isNullValue(element.error.demandOrderCode) || !isNullValue(element.error.consignee) || !isNullValue(element.error.pickupPoint) || !isNullValue(element.error.dropPoint)
...
export function createShipmentParams(orderDetails: any, shipmentData: any, doResponse: any) {
    ...
    if (shipmentData.details.pickupPoint && shipmentData.details.pickupPoint.data) {
        params.shipmentDetails.originLocationCode = shipmentData.details.pickupPoint.data.parentLocationCode;
    }
    if (shipmentData.details.dropPoint && shipmentData.details.dropPoint.data) {
        params.shipmentDetails.destinationLocationCode = shipmentData.details.dropPoint.data.parentLocationCode;
    }
...
export function updateShipmentParams(orderDetails: any, shipmentDeatils: any, shipmentData: any, doResponse: any) {
    ...
    if (shipmentData.details.pickupPoint && shipmentData.details.pickupPoint.data) {
        params.originLocationCode = shipmentData.details.pickupPoint.data.parentLocationCode;
    }
    if (shipmentData.details.dropPoint && shipmentData.details.dropPoint.data) {
        params.destinationLocationCode = shipmentData.details.dropPoint.data.parentLocationCode;
    }
    if (shipmentData.details.length) {
        ...
        shipmentData.details.articles.map((element: any) => {
            if (!isNullValue(element.material) && !isNullValue(element.productQuantity)) {
                shipmentList.push({
                    ...
                    articleName: element.material.label,
                    count: element.materialQuantity,
                    productQuantity: Math.ceil(element.productQuantity),
                    uom: element.unit
                })
            }
        });
...
export function diversionCreateOrderParams(state: any) {
    let userParams = state.userParams;
    let params: any = {
        requestId: userParams.requestId,
        oldFreightOrderCode: userParams.oldFreightOrderCode,
        orderDiversionId: userParams.orderDiversionId,
    ...
    params.shipmentDetails = state.shipmentDeatils.map((element: any) => {
        ...
        if (element.details.demandOrderCode) {
            data.demandOrderCode = element.details.demandOrderCode
        }
        if (element.details.demandOrderCode) {
            // eslint-disable-next-line
