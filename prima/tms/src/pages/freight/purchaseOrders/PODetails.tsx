...
import { poDetailsTitle } from './base/purchaseOrderMessageUtils';
import { poDemandOrderChildrenTableColumn, poDemandOrderTableColumn } from './base/purchaseOrderTemplate';
import { getPOConsigneeDetails, getPODetails, getPOProductDetails, getPOVendorDetails } from './purchaseOrdersApi/purchaseOrderServiceActions';
import { refreshList, setCurrentPage, setRowPerPage, setSelectedElement, setSelectedTab, toggleModal } from './purchaseOrdersRedux/purchaseOrdersActions';
...
function PODetails() {
    ...
    const [PODetails, setPODetails] = React.useState<any>({});
    ...
    useEffect(() => {
        const getList = async () => {
            ...
            let promiseArray: any = [appDispatch(getPODetails(poQueryParams)), appDispatch(getPOConsigneeDetails(poQueryParams)), appDispatch(getPOProductDetails(poQueryParams)), appDispatch(getPOVendorDetails(poQueryParams))]
            Promise.all(promiseArray).then((response: any) => {
                if (response && response[0] && response[0].data && response[0].data[0]) {
                    setPODetails(response[0].data[0])
                } else {
                    setPODetails({})
                }
                    ...
                    <Card className="creat-contract-wrapp creat-wrapp">
                        ...
                        <CardContent className="creat-contract-content">
                            <div className="table-collapse-detail-listing">
                                <TableCollapseList
                                    tableColumns={poDemandOrderTableColumn(onClickDemandButton, onClickApproveButton, onClickModifyAccept, onClickModifyReject, onClickEdit, onClickDeleteButton, PODetails.status)}
                                    currentPage={state.currentPage}
                                    rowsPerPage={state.pageSize}
                                    rowsPerPageOptions={rowsPerPageOptions}
                                    listData={DOList}
                                    childElementKey={'product'}
                                    childrenColumns={poDemandOrderChildrenTableColumn()}
                                    onChangePage={(event: any, page: number) => {
                                        dispatch(setCurrentPage(page));
                                    }}
                                    onChangeRowsPerPage={(event: any) => {
