...
import { refreshList, setCurrentPage, setRowPerPage, setSelectedElement, setSelectedTab } from '../sto/stoRedux/StockTransferActions';
import { soDetailsTitle } from './base/stockOrderMessageUtils';
import { soDemandOrderChildrenTableColumn, soDemandOrderTableColumn } from './base/stockOrderTemplate';
import './SODetails.css';
import { getSOConsigneeDetails, getSODetails, getSOProductDetails, getSOVendorDetails } from './stockOrdersApi/stockOrderServiceActions';
import { toggleModal } from './stockOrdersRedux/stockOrdersActions';
...
function SODetails() {
    ...
    const [SODetails, setSODetails] = React.useState<any>({});
    ...
    useEffect(() => {
        const getList = async () => {
            ...
            let promiseArray: any = [appDispatch(getSODetails(queryParams)), appDispatch(getSOConsigneeDetails(queryParams)), appDispatch(getSOProductDetails(queryParams)), appDispatch(getSOVendorDetails(queryParams))]
            Promise.all(promiseArray).then((response: any) => {
                if (response && response[0] && response[0].data && response[0].data[0]) {
                    setSODetails(response[0].data[0])
                } else {
                    setSODetails({})
                }
                    ...
                    <Card className="creat-contract-wrapp creat-wrapp">
                        ...
                        <CardContent className="creat-contract-content">
                            <div className="table-collapse-detail-listing">
                                <TableCollapseList
                                    tableColumns={soDemandOrderTableColumn(onClickDemandButton, onClickApproveButton, onClickModifyAccept, onClickModifyReject, onClickEdit, onClickDeleteButton, SODetails.status)}
                                    currentPage={state.currentPage}
                                    rowsPerPage={state.pageSize}
                                    rowsPerPageOptions={rowsPerPageOptions}
                                    listData={DOList}
                                    childElementKey={'product'}
                                    childrenColumns={soDemandOrderChildrenTableColumn()}
                                    onChangePage={(event: any, page: number) => {
                                        dispatch(setCurrentPage(page));
                                    }}
                                    onChangeRowsPerPage={(event: any) => {
