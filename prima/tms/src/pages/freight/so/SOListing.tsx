...
import { RegisterJobs, rowsPerPageOptions, soOptions, sourceTypeEnum, stockTransferStatusEnum, stockTransferStatusTab } from "../../../base/constant/ArrayList";
import { bulkProcessTitle } from '../../../base/constant/MessageUtils';
import { EditBulkSO, SODetailsUrl, SOListingUrl } from "../../../base/constant/RoutePath";
...
import { SO_Filters } from "../../../moduleUtility/FilterUtils";
import { showAlert } from '../../../redux/actions/AppActions';
import { refreshList, setCheckedListResponse, setCurrentPage, setRowPerPage, setSelectedTab, showLoading, toggleFilter, toggleModal } from "../so/stockOrdersRedux/stockOrdersActions";
import { soListingTitle } from './base/stockOrderMessageUtils';
import { soBulkTableColumn, soChildColumn, soTableColumn } from "./base/stockOrderTemplate";
import SOFilters from './SOFilters';
import './SOListing.css';
import { getSOList } from "./stockOrdersApi/stockOrderServiceActions";
import { setSelectedElement } from './stockOrdersRedux/stockOrdersActions';
import StockOrdersReducer, { STOCK_ORDER_STATE } from './stockOrdersRedux/StockOrdersReducer';
...
function SOListing() {
    ...
    const [filterState, addFiltersQueryParams, removeFiltersQueryParams] = useSearchParams(SO_Filters);
    const [state = STOCK_ORDER_STATE, dispatch] = useReducer(StockOrdersReducer, STOCK_ORDER_STATE);
    ...
    useEffect(() => {
        const getList = async () => {
            ...
            appDispatch(getSOList(dispatch, queryParams, id ? id : state.selectedTabName));
        }
        getList();
    ...
    return (
        <div className="so-listing-wrapper">
            ...
            <BulkUploadDialog
                ...
                jobName={RegisterJobs.STOCK_ORDER}
                onClose={() => {
            ...
            <div className="bill-tab tab-nav">
                {isBulk ? (
                    ...
                    if (newValue !== state.selectedTabIndex) {
                        ...
                        history.replace({
                            pathname: SOListingUrl + stockTransferStatusTab[newValue],
                    ...
                    {!isMobile &&
                        ...
                        >{state.listData && state.listData.length > 0 &&
                            <Button
                                ...
                                onClick={() => {
                                    if (countSelectedSOs() === 0) {
                                        appDispatch(showAlert("Please select atleast one SO.", false));
                                        return;
                                    }
                                    else {
            ...
            >
                        ...
                        <Chips
                            ...
                            onDelete={() => {
                                ...
                                if (element === "fromDate" || element === "toDate" || element === "query") {
                                    ...
                                    //let thirdKey = element === "distanceMin" ? "distanceMax" : "distanceMin";
                ...
                {!isBulk ?
                    isMobile ?
                        <ExpendableCardList
                            listData={state.listData}
                            tableColumns={soTableColumn(onClickViewButton, onClickSourceButton, state.selectedTabName)}
                            isNextPage={state.pagination && state.pagination.next}
                            childElementKey={'product'}
                            childTableColumns={soChildColumn()}
                            onReachEnd={() => {
                    ...
                    <>
                        <TableList
                            tableColumns={soBulkTableColumn(handleChecks, handleAllChecks, allValue, onClickSourceButton, state.selectedTabName)}
                            currentPage={state.currentPage}
                            rowsPerPage={state.pageSize}
                            rowsPerPageOptions={rowsPerPageOptions}
                            totalCount={state.pagination && state.pagination.count}
    ...
    function onClickSourceButton(element: any) {
        element && history.push({
            pathname: SODetailsUrl + element.id,
