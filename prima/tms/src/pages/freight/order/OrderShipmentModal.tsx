...
import {
    ...
    remarkLabel, ShipmentTagLabel, ShipmentTagPlaceholder, tatLabelWithoutUnit, tatPlaceholder, VolumeLabel, volumePlaceholder
...
import { addShipment, editShipment, getOrderLocations, getOrderZoneLocations } from '../../../serviceActions/OrderServiceActions';
import { createShipmentParams, RenderArticles, updateShipmentParams, validateShipmentDetails } from "../raiseOrders/RaiseOrderViewUtils";
...
function OrderShipmentModal(props: OrderShipmentModalProps) {
    const { open, onClose, selectedElement, onSuccess, shipmentDeatils, freightOrderCode, freightType, doResponse, doMaterialResponse, tagListResponse } = props;
    ...
    useEffect(() => {
        const getDetails = async () => {
            if (selectedElement) {
                ...
                if (selectedElement.laneCode || (selectedElement?.originZoneCode && selectedElement?.destinationZoneCode)) {
                    let orderLocationsResponse = (selectedElement?.originZoneCode && selectedElement?.destinationZoneCode)
                        ? await appDispatch(getOrderZoneLocations({ origin_zone_code: selectedElement?.originZoneCode, destination_zone_code: selectedElement?.destinationZoneCode, serviceabilityModeCode: selectedElement.serviceabilityModeCode }))
                        : await appDispatch(getOrderLocations({ laneCode: selectedElement.laneCode, freightOrderCode }));
                    //let orderLocationsResponse = await appDispatch(getOrderLocations({ laneCode: selectedElement.laneCode, freightOrderCode }));
                ...
                if (shipmentDeatils) {
                    ...
                    let details = {
                        partnerName: selectedElement.label,
                        partner: {
                        ...
                        shipmentTag: shipmentDeatils.shipmentTag,
                        shipmentTagValue: shipmentDeatils.shipmentTag ? {
                            label: shipmentDeatils.shipmentTag,
                            value: shipmentDeatils.shipmentTag,
                        } : "NA",
                    ...
                    shipmentDeatils.articleDetails && shipmentDeatils.articleDetails.forEach(async (element: any, index: number) => {
                        ...
                        if (element.articleName) {
                            if (doMaterialResponse) {
                                ...
                                data.refDocketNumber = element.refDocketNumber;
                                data.invoiceNumber = element?.invoiceNumber;
                                ...
                                if (!(materialUnitCountDetails?.isBulk)) {
                                    if (materialUnitCountDetails?.weightUom && (element.uom === materialUnitCountDetails?.weightUom)) {
                                        data.multiplyingFactor = materialUnitCountDetails?.weight
                                    }
                                    else if (materialUnitCountDetails?.volumeUom && (element.uom === materialUnitCountDetails?.volumeUom)) {
                    ...
                    saveDetails({
                        ...
                        tatValue: tatOptions[0],
                        sourceNUmber: selectedElement && selectedElement.sourceNUmber
                    }, {
                        partner: "",
                    })
                }
    ...
    return (
        <ModalContainer
            ...
            open={open}
            styleName="order-shipment-modal shipment-modal-width"
            onApply={() => {
                let validate = validateShipmentDetails([details], { label: freightType }, doResponse);
                if (validate === true) {
                    addNewShipment();
                } else {
                    setDetails({
                    ...
                    <div className=" shipment_Details row">
                        <div className="col-md-12 col-lg-12">
                            <div className="custom-form-row row align-items-end">
                                {doResponse && doResponse.sourceNumber && (
                                    <div className="col-md-6 form-group">
                                        <EditText
                                            ...
                                            // value={details.details.sourceNumber}
                                            value={details.details.sourceNumber || doResponse.sourceNumber}
                                ...
                                <div className="col-md-6 form-group">
                                    <AutoSuggest
                                        ...
                                        value={selectedElement.partnerName}
                                        isDisabled={true}
                                ...
                                <div className="form-group col-md-6">
                                    <AutoComplete
                                        ...
                                        value={details.details.shipmentTagValue}
                                        error={details.error.shipmentTag}
                                        options={tagListResponse}
                                        onChange={(element: OptionType) => {
                                            saveDetails({
                                                shipmentTagValue: element,
                                                shipmentTag: "",
                                            }, {
                                                shipmentTag: ""
                                            })
                                ...
                                {/* <div className="form-group col-md-6">
                                    <EditText
                                        ...
                                        maxLength={25}
                                        value={details?.details && details?.details?.waybillNumber}
                                        onChange={(text: string) => {
                                            saveDetails({
                                                waybillNumber: text.trim()
                                            }, {
                                                waybillNumber: ""
                                            })
                                        }}
                    ...
                    <div className="shipmentDetails row">
                        {details.details.articles && details.details.articles.map((element: any, index: number) => (
                            <RenderArticles
                                ...
                                // doMaterialList={DoMaterials || doMaterialResponse}
                                doMaterialList={doMaterialResponse && setAutoCompleteListWithData(doMaterialResponse, "materialName", "materialCode")}
                                onAdd={() => {
                                    saveDetails({
                                    ...
                                    if (!articleData.isBulk) {
                                        if (value.value === articleData?.weightUom) {
                                            ...
                                            if (response && response.value) {
                                                ...
                                                uomError = "Enter valid UOM";
                                                currentRow.multiplyingFactor = undefined
                                            }
                                        }
                                    }
                                    const articles = details.details.articles.map(
                                ...
                                onChangeReferenceDocketNo={(value: any, ReferenceDocketNoIndex: number) => {
                                    let articles = details.details.articles.map(
                                        (data: any) => ((data.index === ReferenceDocketNoIndex) ? {
                                            ...data,
                                            refDocketNumber: value,
                                        } : data))
                                    saveDetails({
                                        ...details.details,
                                        articles: articles,
