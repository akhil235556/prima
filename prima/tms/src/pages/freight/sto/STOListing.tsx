...
import { RegisterJobs, rowsPerPageOptions, sourceTypeEnum, stockTransferStatusEnum, stockTransferStatusTab, stoOptions } from "../../../base/constant/ArrayList";
import { bulkProcessTitle } from '../../../base/constant/MessageUtils';
import { EditBulkSTO, STODetailsUrl, STOListingUrl } from "../../../base/constant/RoutePath";
...
import { STO_Filters } from "../../../moduleUtility/FilterUtils";
import { showAlert } from '../../../redux/actions/AppActions';
import { stoListingTitle } from "./base/stockTransferOrderMessageUtils";
import { stoBulkTableColumn, stoChildColumn, stoTableColumn } from "./base/stockTransferOrderTemplate";
import { getSTOList } from "./stockTransferOrdersApi/stoServiceActions";
import STOFilters from './STOFilters';
import { refreshList, setCheckedListResponse, setCurrentPage, setRowPerPage, setSelectedElement, setSelectedTab, showLoading, toggleFilter, toggleModal } from "./stoRedux/StockTransferActions";
...
function STOListing() {
    ...
    const [filterState, addFiltersQueryParams, removeFiltersQueryParams] = useSearchParams(STO_Filters);
    ...
    useEffect(() => {
        const getList = async () => {
            ...
            appDispatch(getSTOList(dispatch, queryParams, id ? id : state.selectedTabName));
        }
        getList();
    ...
    return (
        <div className="so-listing-wrapper">
            ...
            <BulkUploadDialog
                ...
                jobName={RegisterJobs.STOCK_TRANSFER_ORDER}
                onClose={() => {
                ...
                <div className="bill-tab tab-nav">
                    {isBulk ? (
                        ...
                        if (newValue !== state.selectedTabIndex) {
                            ...
                            history.replace({
                                pathname: STOListingUrl + stockTransferStatusTab[newValue],
                            ...
                            {!isMobile &&
                                ...
                                >{state.listData && state.listData.length > 0 &&
                                    <Button
                                        ...
                                        onClick={() => {
                                            if (countSelectedSTOs() === 0) {
                                                appDispatch(showAlert("Please select atleast one STO.", false));
                                                return;
                                            }
                                            else {
            ...
            >
                ...
                {!isBulk ?
                    isMobile ?
                        <ExpendableCardList
                            listData={state.listData}
                            tableColumns={stoTableColumn(onClickViewButton, onClickSourceButton, state.selectedTabName)}
                            isNextPage={state.pagination && state.pagination.next}
                            childElementKey={'product'}
                            childTableColumns={stoChildColumn()}
                            onReachEnd={() => {
                        ...
                        <TableCollapseList
                            ...
                            childElementKey={'product'}
                            childrenColumns={stoChildColumn()}
                            listData={state.listData}
                        ...
                        /> : <>
                        <TableList
                            tableColumns={stoBulkTableColumn(handleChecks, handleAllChecks, allValue, onClickSourceButton, state.selectedTabName)}
                            currentPage={state.currentPage}
                            rowsPerPage={state.pageSize}
                            rowsPerPageOptions={rowsPerPageOptions}
                            totalCount={state.pagination && state.pagination.count}
    ...
    function onClickSourceButton(element: any) {
        element && history.push({
            pathname: STODetailsUrl + element.id,
