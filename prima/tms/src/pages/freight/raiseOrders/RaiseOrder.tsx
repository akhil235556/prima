...
import { ArrowRightAlt, ClearAll, KeyboardBackspace, Publish } from "@material-ui/icons";
import React, { useContext, useEffect } from "react";
import { shallowEqual, useDispatch, useSelector } from "react-redux";
import { useHistory, useParams } from "react-router";
import { isNullOrUndefined } from "util";
import { FreightType, ftlServiceabilityModeTypeList, JobFileType, ptlModeTypeList, RegisterJobs, serviceabilityModeTypeList } from "../../../base/constant/ArrayList";
import {
  contractId, destinationLabel, destinationPlaceHolder, laneLabel, lanePlaceholder,
  modeLabel,
  modePlaceholder,
  modLabel, modPlaceholder, orderBulkFileTypes, orderUploadFile, originLabel, originPlaceHolder, referenceIdLabel,
  referenceIdPlaceholder, remarkLabel, transporterLabel, transporterPlaceHolder, uploadExcelFileTypes, vehicleTypeLabel, vehicleTypePlaceholder
...
import {
  ...
  setPartnerList, setUserParams, toggleBulkUpload, toggleOrderDeatils
} from '../../../redux/actions/RaiseIndentAction';
import { getLanePrice, getPtlZoneContract } from "../../../serviceActions/FrightRateServiceAction";
import { searchLane, searchLanePartner } from '../../../serviceActions/LaneServiceActions';
import { searchZoneLocationList, setZoneOrigin } from "../../../serviceActions/LocationServiceActions";
...
import { getDOList } from "../demandOrders/demandOrdersApi/demandOrderServiceActions";
import { Context } from "./CreateOrder";
import "./RaiseOrder.css";
import { validateIndentData } from "./RaiseOrderViewUtils";
...
function RaiseOrder() {
      ...
      if (state.userParams && state.userParams.freightType && state.userParams.partner
        ...
        appDispatch(getLanePrice(params)).then((response: any) => {
          ...
          if (response) {
            ...
            } else if (!response.lanePrice && response.isKg) {
              userParams.lanePrice = "0";
              setLaneVisible(false);
            } else {
  ...
  useEffect(() => {
    if (state.userParams?.ptlMode?.value === "zone" && state.userParams?.partner?.value && state.userParams?.serviceabilityModeCode?.value) {
      appDispatch(getPtlZoneContract({ partnerCode: state.userParams?.partner?.value, contractMode: "zone", contractStatus: "ACTIVE", contractType: "PTL", serviceabilityModeCode: state.userParams?.serviceabilityModeCode?.value })).then((response: any) => {
        setValues({ contractCode: response?.contracts?.[0]?.contractCode });

        setShowContract(!!response?.contracts?.[0]?.contractCode);
      })
    }
    dispatch(setUserParams({
      ...state.userParams,
  ...
  return (
    <div className="freight-order-wrapper">
      ...
      <BulkUploadDialog
        ...
        modalMessage={JobFileType.STANDARD_FILE === fileType ? uploadExcelFileTypes : orderBulkFileTypes}
        acceptFiles={JobFileType.STANDARD_FILE === fileType ? ".xls,.xlsx" : ".csv,.xls,.xlsx"}
          ...
          {loading ?
            ...
            <CardContent className="creat-contract-content raise-contract-detail">
              <div className="row custom-form-row align-items-end">
                    ...
                    onChange={(element: OptionType) => {
                      if (id) {
                        ...
                        dispatch(setValues({
                          ...
                          partner: undefined,
                          lanePrice: "",
                          ptlMode: element.value === FreightType.PTL && userInfo?.locationName === "All"
                            ? {
                              label: "Lane",
                              value: "lane",
                            } : undefined,
                ...
                {state.userParams?.freightType?.value === FreightType.PTL &&
                  <div className="form-group col-md-6">
                    <AutoComplete
                      ...
                      mandatory
                      isDisabled={state.userParams?.freightType?.value === FreightType.PTL && (userInfo?.locationName === "All" || !state.zoneEnabled)}
                      error={state?.error?.ptlMode}
                      value={state?.userParams && state?.userParams?.ptlMode}
                      onChange={(element: OptionType) => {
                        setValues({
                          ptlMode: element,
                          origin: undefined,
                          ...
                          contractCode: undefined,
                          originZoneName: undefined,
                          destinationName: undefined,
                          originZoneCode: undefined,
                          serviceabilityModeCode: undefined,
                          tat: undefined,
                        })
                        setShowContract(false);
                      }}
                ...
                {(state.userParams?.freightType?.value === FreightType.PTL && state.userParams?.ptlMode?.value === "zone") ?
                  <>
                    <div className="col-md-6 form-group">
                      <AutoSuggest
                        ...
                        onSelected={(element: OptionType) => {
                          !isObjectEmpty(element) && appDispatch(setZoneOrigin({ partnerCode: element.value })).then((response: any) => {
                            setValues({
                              originZoneCode: response?.zoneCode, originZoneName: response?.zoneName,
                              partner: element,
                    ...
                    <div className="form-group col-md-6">
                      <EditText
                        ...
                        value={state?.userParams?.originZoneName}
                    ...
                    <div className="form-group col-md-6">
                      <AutoSuggest
                        ...
                        value={state?.userParams?.destinationName}
                        error={state?.error?.destination}
                        suggestions={locationList}
                        dependencies={[state?.userParams?.destinationName]}
                        handleSuggestionsFetchRequested={({ value }: any) => {
                          if (value.length > autosuggestSearchLength) {
                            getLocationList(value.trim());
                          }
                        }}
                ...
                {!(state.userParams?.freightType?.value === FreightType.PTL && state.userParams?.ptlMode?.value === "zone") && <div className="col-md-6 form-group">
                  <AutoSuggest
                    mandatory
                    label={transporterLabel}
                    placeHolder={transporterPlaceHolder}
                ...
                {laneVisible && state.userParams?.ptlMode?.value !== "zone" &&
                  <div className="form-group col-md-6">
                    <NumberEditText
                      label={"Lane Price ( \u20B9 )"}
              ...
              onClick={() => {
                ...
                if (validate === true) {
                  let serviceablityParams: any = {
                    ...
                    partnerCode: state.userParams?.partner?.value,
                    serviceabilityModeCode: state.userParams?.serviceabilityModeCode?.value,
                    originCode: state.userParams?.originZoneCode,
                    destinationCode: state.userParams?.destination?.value,
                  }
  ...
  function setValues(params: any) {
    if ((state.userParams?.freightType?.value === FreightType.PTL || params?.freightType?.value === FreightType.PTL)
      && (userInfo?.locationName === "All" || !state.zoneEnabled)) {
      params.ptlMode = ptlModeTypeList[0];
    }
    dispatch(setUserParams({
      ...state.userParams,
      ...params
  ...
  function getLocationList(text: string) {
    appDispatch(searchZoneLocationList({ query: text, partner_code: state.userParams?.partner?.value })).then((response: any) => {
      setLocationList(setAutoCompleteListWithData(response, "zoneName", "zoneCode"));
    })
  }
