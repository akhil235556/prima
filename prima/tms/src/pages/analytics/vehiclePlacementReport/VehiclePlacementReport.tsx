...
import { vehiclePlacementFilters } from "../../../moduleUtility/FilterUtils";
import { showAlert } from '../../../redux/actions/AppActions';
import { refreshList, setCurrentPage, setResponse, setRowPerPage, setSelectedElement, toggleFilter, toggleModal } from '../../../redux/actions/VehiclePlacementActions';
import VehiclePlacementReducer, { VEHICLE_PLACEMENT_STATE } from '../../../redux/reducers/VehiclePlacementReducer';
import { getCountList, getCsvLink, getVehiclePlacementList } from '../../../serviceActions/VehiclePlacementServiceActions';
import { vehiclePlacementTableColumns } from "../../../templates/AnalyticsTemplates";
...
import VehiclePlacementReportFilters from "./VehiclePlacementReportFilters";

function VehiclePlacementReport() {
  const appDispatch = useDispatch();
  const history = useHistory();
  const [state = VEHICLE_PLACEMENT_STATE, dispatch] = useReducer(VehiclePlacementReducer, VEHICLE_PLACEMENT_STATE);
  const [loading, setLoading] = React.useState(false);
  const [countLoading, setCountLoading] = React.useState(false);
  const [count, setCount] = React.useState<any>({});
  const [filterState, addFiltersQueryParams, removeFiltersQueryParams] = useSearchParams(vehiclePlacementFilters);
  ...
  useEffect(() => {
    const getList = async () => {
      ...
      setLoading(true);
      appDispatch(getVehiclePlacementList(queryParams)).then((response: any) => {
  ...
  return (
    <div className={isObjectEmpty(count) ? "placement-efficiency analytics-report analytics-report-wrapp" : "placement-efficiency analytics-report"}>
      ...
      <Filter
        pageTitle="Vehicle Placement Report"
      >
        ...
        {loading ? <ListingSkeleton /> :
          (isMobile ?
            <CardList
              ...
              tableColumns={vehiclePlacementTableColumns(onClickLaneCode)}
...
export default VehiclePlacementReport;
