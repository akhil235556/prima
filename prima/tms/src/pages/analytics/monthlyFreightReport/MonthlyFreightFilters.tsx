...
import { orderStatusList } from '../../../base/constant/ArrayList';
...
interface MonthlyFreightFiltersProps {
    open: boolean,
    onClose: any,
    onApplyFilter: any,
    filerChips: any,
...
function MonthlyFreightFilters(props: MonthlyFreightFiltersProps) {
                ...
                <div className="form-group">
                    <AutoComplete
                        ...
                        value={filterValues.statusName ? {
                            ...
                            value: filterParams.statusName
                        } : undefined}
                        options={orderStatusList}
                        onChange={(value: OptionType) => {
                            setValues({ statusName: value.label }, { statusName: value.label.toUpperCase() })
                        }}
                    />
                </div>
                <div className="form-group">
                    <AutoSuggest
                        ...
                        onSelected={(element: OptionType) => {
                            setValues({ destinationLocationName: element.label }, { originLocationCode: element.value });
                        }}
                        onChange={(text: string) => {
                            setValues({ destinationLocationName: text }, { originLocationCode: "" });
                        }}
                    />
                </div>
    ...
    function onApply() {
        if (!isFilterNullValue(filterValues.partnerName) && isNullValue(filterParams.partnerCode)) {
            ...
            return;
        } else if (!isFilterNullValue(filterValues.destinationLocationName) && isNullValue(filterParams.originLocationCode)) {
...
export default MonthlyFreightFilters;
