...
class BaseDao(ABC):
    ...
    def count(self, filters, wildcard_field, wildcard_value):
        ...
        # fields should be in order of table definition
        """
        base_query = sql.SQL("select count(*) from {table}  ").format(
            table=sql.Identifier(self.table_name)
        )
        if filters and self.is_subset(filters.keys(), self.fields):

            where_clause, where_placeholder_values = self.where(**filters)
            where_query = sql.SQL("{where_clause}").format(
                where_clause=where_clause
                )
            ...
            resp = self.execute(base_query, result=True)
        else:
            raise AppException(
                "mismatch in attributes and fields of "
                "client table. {} attributes not present in schema".format(
    ...
    def paginated_query(self, filters, wildcard_field=None, wildcard_value=None, fields=None, page=1, page_size=10):
        ...
        if self.is_subset(filters.keys(), self.fields):
            ...
            if filters:
                ...
                where_clause, where_placeholder_values = self.where(**filters)
                logger.info(
                    f"where_clause>>{where_clause} >>>>>>where_placeholder_values>>>>> {where_placeholder_values}")
                where_query = sql.SQL("{where_clause} ").format(
                    where_clause=where_clause
                )
            ...
            resp = self.execute(query, result=True)
        else:
            raise AppException(
                "mismatch in attributes and fields of "
                "client table. {} attributes not present in schema".format(
    ...
    @staticmethod
    def is_timestamp(value, key=None, datetime_format="%Y-%m-%d %I:%M:%S.%f+05:30"):
        display_key = str(key)
        try:
            if bool(datetime.datetime.strftime(value, datetime_format)):
                raise AppException(f"{display_key} is invalid timestamp (valid format = {format})")
        except ValueError:
            raise AppException(f"{display_key} is not of type datetime")

    @staticmethod
    def is_decimal(value, key=None):
        display_key = str(key or value)
        if not (isinstance(value, int) or isinstance(value, float)):
            raise AppException("{} is not of type float/int".format(display_key))

    @staticmethod
    def set_page_and_page_size(request_dict):

        if "page" in request_dict.keys():
            if not request_dict.get("page", "").isdigit():
                raise AppException("page is not of type int")
            else:
                request_dict["page"] = int(request_dict["page"])

        if "page_size" in request_dict.keys():
            if not request_dict.get("page_size", "").isdigit():
                raise AppException("page_size is not of type int")
            else:
                request_dict["page_size"] = int(request_dict["page_size"])
        return request_dict
