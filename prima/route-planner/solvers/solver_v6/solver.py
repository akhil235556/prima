...
from solvers.solver_v6.input_processor import SolverInputDataProcessor
from solvers.solver_v6.solution import SolverSolutionProvider
...
class SolverVRP(object):
    ...
    def __init__(self, orders, vehicles, agg, all_vehicles, request_id=None, timestamp=None, config=None,
    ...
    def _init_solver_data(self, orders, vehicles, agg, all_vehicles, config):
        pd.set_option('display.max_columns', None)
        if not config.sequential_bool:
            self.set_vehicle_index(vehicles)
            vehicles['No. of vehicles'] = SolverVRP.repeat_vehicles(vehicles, orders)
        data, vehicle_data_model, all_vehicle_data_model = SolverInputDataProcessor(orders=orders, vehicles=vehicles,
                                                                                    agg=agg, all_vehicles=all_vehicles,
    ...
    def set_routing_params(self):
        self._solver_init_data = self._init_solver_data(self._orders, self._vehicles, self._agg, self._all_vehicles,
                                                        self._solver_config)
    ...
    def add_vehicle_fixed_cost_dimension(self):
        ...
        if not fixed_costing:
            for vehicle_id in range(self._solver_init_data['num_vehicles']):
                _vehicle_cost_per_km_callback_method = self.vehicle_cost_per_km_callback(
                    self._solver_init_data['distance_matrix'],
                    1,
                    self._solver_init_data['fixed_charges'][vehicle_id])
                _vehicle_cost_per_km_callback_method_index = self._routing_model.RegisterTransitCallback(
                    _vehicle_cost_per_km_callback_method
    ...
    def execute(self):
        ...
        if solution:
            solution = SolverSolutionProvider(
