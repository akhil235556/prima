...
import pandas as pd
import logging
# from route_planner.utils.utils import pincode_to_coord, coordinates_to_matrix, centroid
logger = logging.getLogger(__name__)

class SolverV4InputDataProcessor(object):
    ...
    def __init__(self, orders, vehicles, products, config, const_int_multiplier):
        ...
        SolverV4InputDataProcessor.type_cast_to_str(self.orders, self.task_str_header)
        SolverV4InputDataProcessor.type_cast_to_str(self.vehicles, self.vehicles_str_header)
        SolverV4InputDataProcessor.type_cast_to_str(self.products, self.products_str_header)
    ...
    # @staticmethod
    # def get_distance_matrix_1(orders, pincode_column, dedicated):
    #     pincode_list = orders[pincode_column].values.tolist()
    #     unique_pincodes = list(set(pincode_list))
    #     coordinates = pincode_to_coord(unique_pincodes)
    #     matrix_df = coordinates_to_matrix(coordinates, unique_pincodes)
    ...
    #
    # @staticmethod
    # def get_distance_matrix_2(orders, dedicated):
    #     central_coordinates = list()
    #     city_list = list()
    #     for key, value in orders.groupby(['from_city', 'to_city']):
    #         coord_list = [[x] + [y] for x, y in zip(value['from_longitude'].tolist(), value['from_latitude'].tolist())]
    #         central_point = centroid(coord_list)
    #         central_coordinates.append(central_point)
    #         city_list.append(key[1])
    #     if len(central_coordinates) < 2:
    #         central_coordinates.append(central_coordinates[0])
    #         city_list.append(city_list[0])
    #     matrix_df = coordinates_to_matrix(central_coordinates, city_list)
    #     matrix = matrix_df.values.tolist()
    #     if not dedicated:
    #         for k in range(len(matrix)):
    #             matrix[k][0] = 0
    ...
    def create_exclusion_data(orders, products):
        ...
        if not prod_with_exclusive.empty:
            # convert csv to nested list
            prod_with_exclusive['exc_tags'] = prod_with_exclusive['exclusive_tags'].map(lambda x: str(x).split(",")).to_list()
            ...
            prod_with_tag['tags'] = prod_with_tag['sku_tag'].map(lambda x: str(x).split(",")).to_list()
            ...
            for idx, (order_id, order_sku) in enumerate(order_id_sku_tuple):
                ...
                if order_sku in exclusion_sku_dict.keys():
                    order_id_exclusion_list = [ex_skus for ex_skus_list in exclusion_sku_dict[order_sku] for ex_skus in
                                               SolverV4InputDataProcessor.map_sku_to_order_id(orders, ex_skus_list)]
