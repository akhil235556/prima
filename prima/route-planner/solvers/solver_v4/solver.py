...
from solvers.solver_v4.input_processor import SolverV4InputDataProcessor
from solvers.solver_v4.solution import SolverSolutionProviderV4
...
class SolverV4(object):
    ...
    def __init__(self, orders, vehicles, products=None, request_id=None, timestamp=None, config=None,
        ...
        if not config:
            config = SkuFixedSolverConfigurationV4.get_default_sku_fixed_configuration()
        ...
        if not vehicles.empty:
            ...
            self._vehicles = self.repeat_vehicles()
    ...
    def repeat_vehicles(self):
        self._vehicles.rename(columns={'No. of vehicles': 'num_vehicles'}, inplace=True)
        return vehicle_multiplier(self._vehicles, self._orders, return_df=True)
    ...
    def _init_solver_data(orders, vehicles, products, extra_products, config, int_multiplier):
        # get data for solver
        data = SolverV4InputDataProcessor(orders, vehicles, products, config, int_multiplier).create_data_model()
        logger.info(data)
    ...
    def set_solver_variables(self):
        ...
        for i in self._solver_init_data['orders'].get('id'):
            for j in self.get_vehicles_for_order(i):
                x[(i, j)] = self._routing_model.IntVar(0, 1, '')

        # y[j] = 1 if bin j is used.
        y = {}
        for j in self._solver_init_data['vehicles'].get('id'):
            y[j] = self._routing_model.IntVar(0, 1, '')
        return x, y
    ...
    def add_vehicle_min_load_capacity_constraint(self, x, y):
        ...
        m = f"The amount of load packed in each truck should be at-least >= lower bound of it's capacity.(hard bound)"
        logger.info(f"Constraint Added: '{m}'")
        for j in self._solver_init_data['vehicles'].get('id'):
            self._routing_model.Add(
                sum(x[(i, j)] * self._solver_init_data['orders']['demand_load'][i] for i in self._solver_init_data['orders'].get('id') if j in self.get_vehicles_for_order(i)) >= y[j] *
    ...
    def add_vehicle_min_volume_capacity_constraint(self, x, y):
        ...
        m = f"The amount of volume packed in each truck should be at-least >= lower bound of it's capacity.(hard bound)"
        logger.info(f"Constraint Added: '{m}'")
        for j in self._solver_init_data['vehicles'].get('id'):
            self._routing_model.Add(
                sum(x[(i, j)] * self._solver_init_data['orders']['demand_volume'][i] for i in self._solver_init_data['orders'].get('id') if j in self.get_vehicles_for_order(i)) >= y[j] *
    ...
    def add_exclusion_constraint(self, x, y):
        m = "Exclusion"
        logger.info(f"Constraint Added: '{m}'")
        constraint_usage = False
        logger.info(self._solver_init_data['orders'].get('exclusion_orders'))
            ...
            for exclusion_i in exclusion_list:
                for j in self.get_vehicles_for_order(i):
                    ...
                    if j in self.get_vehicles_for_order(exclusion_i):
                        b = x[(exclusion_i, j)]
                        self._routing_model.Add(a + b <= 1)
                        # self._routing_model.Add(sum([a, b]) <= 1)
                        # self._routing_model.AddBoolOr({a.Not(), b.Not()})
    ...
    def add_objective(self, x, y):
        ...
        self._routing_model.Minimize(self._routing_model.Sum([self._routing_model.Sum(c) for c in veh_cost]))
        # self._routing_model.Minimize(self._routing_model.Sum([y[j] * (self._solver_init_data['vehicles'].get('fixed_charges')[j]) for j in self._solver_init_data['vehicles'].get('id')]))

    def run(self):
        summary_dict = dict(city=list(),
                            obj=list(),
                            discrete_var=list(),
                            ...
                            total_wt=list(),
                            total_vol=list(),
                            avg_wt_utilization=list(),
                            avg_vol_utilization=list(),
                            per_unit_wt_cost=list())

        # cp_solver = cp_model.CpSolver()

        # Sets a time limit of 190 seconds.
        # cp_solver.parameters.max_time_in_seconds = 180
        # self._routing_model.EnableOutput()
        self._routing_model.SetTimeLimit(500000)

        if self._solver_config.num_of_threads:
            logger.info(f"Using NUM_OF_THREADS: {int(self._solver_config.num_of_threads)}")
            # Set Number of threads
            self._routing_model.SetNumThreads(int(self._solver_config.num_of_threads))

        # cp_solver.parameters.log_search_progress = True
        status = self._routing_model.Solve()
        # status = cp_solver.Solve(self._routing_model)
        summary_dict['obj'].append(self._routing_model.Objective().Value())
        summary_dict['discrete_var'].append(self._routing_model.NumVariables())
        summary_dict['solution_time'].append(self._routing_model.wall_time()/60000)
        if not self._routing_model.Objective().Value() == 0:
            summary_dict['opt_gap'].append(((self._routing_model.Objective().Value() - self._routing_model.Objective().BestBound()) / self._routing_model.Objective().Value()) * 100)
        return self._routing_model, status, summary_dict
    ...
    def execute(self):
        ...
        if config.sku_exclusion_constraint:
            usage = self.add_exclusion_constraint(x, y)
            if usage:
                num_constraint += 1
        ...
        # solution
        cp_solver, status, summary = self.run()

        summary['num_constraints'].append(num_constraint)
        ...
        if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
            ...
            if status == pywraplp.Solver.FEASIBLE:
                ...
                stat = "Optimal"
            #  Generate output
            summary['status'].append(stat)

            solution = SolverSolutionProviderV4(
                solver_output, self._orders, self._vehicles, self._products, config,
                from_city=self.from_city, to_city=self.to_city, status=stat, summary=summary)
            solution.solve()
            return solution.get_aggregated_output(), solution.get_output(), solution.get_confusion_matrix(), stat, solution.get_summary()
        else:
            sum = []
            routes = []
            cm = []
            ag = []
            return ag, routes, cm, stat, sum
    ...
    def dummy_output(self, cp_solver, status, x, y):
        ...
        #
        detail['solver_time_s'] = cp_solver.WallTime()/60
    ...
    def solver_output(self, cp_solver, status, x, y):
        ...
        for j in self._solver_init_data['vehicles'].get('id'):
            if y[j].solution_value() > 0:
                ...
                bin_volume = 0
                for i in self._solver_init_data['orders'].get('id'):
                    if j in self.get_vehicles_for_order(i) and x[i, j].solution_value() > 0:
                        bin_items.append(i)
