...
class SolverSolutionProviderTSP(object):
    ...
    def _get_routes(data, manager, routing, solution, config, status, from_city, to_city):
        ...
        for vehicle_id in range(data['num_vehicles']):
            ...
            route_id = SolverSolutionProviderTSP.generate_uuid()
            while not routing.IsEnd(vehicle_route_index):
                ...
                if int(location_id) == 0:
                    node_cost = data['fixed_charges'][vehicle_id]
                elif not nan_check(data['cost_per_kg'][0]):
                    node_cost += load * data['cost_per_kg'][vehicle_id] / 1000
                elif not nan_check(data['cost_per_km'][0]):
                    node_cost += node_distance / 1000 * data['cost_per_km'][vehicle_id] / 1000
            ...
            if config.daily_run and not vehicles_daily_limit_list[-1] < 1:
                ...
                cal_node_time_list = SolverSolutionProviderTSP.calculate_time_taken(node_distance_list,
    ...
    @staticmethod
    def _get_summary_output(routes, agg_output, solution, num_constraints, status, solver_time, to_city):
        summary_dict = dict(
            city=list(),
            vehicle_type=list(),
            objective=list(),
        ...
        route_dict = routes[0]
        summary_dict['city'].append(to_city)
    ...
    def solve(self):
        ...
        summary_output = self._get_summary_output(routes, aggregated_output, self._solution, self.num_constraints,
                                                  self.status, self.solver_time, self.to_city)
    ...
    def infeasible_solution(self):
        ...
        summary_dict['city'].append(self.to_city)
        summary_dict['vehicle_type'].append(self._data['cap_name'])
        summary_dict['status'].append('FAILURE')
