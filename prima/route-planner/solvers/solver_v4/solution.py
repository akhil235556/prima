...
class SolverSolutionProviderV4(object):

    def __init__(self, solver_output, orders, vehicles, products, config, from_city=None, to_city=None, status=None, summary=None):
    ...
    @staticmethod
    def _get_summary(output_dict, summary, to_city):
        summary['city'].append(to_city)
        summary['total_wt'].append(sum(output_dict.get('vehicle_load')))
        summary['total_vol'].append(sum(output_dict.get('vehicle_volume')))
        summary['avg_wt_utilization'].append(sum(output_dict.get('weight_utilization'))/len(
            output_dict.get('weight_utilization')))
        summary['avg_vol_utilization'].append(sum(output_dict.get('volume_utilization')) / len(
            output_dict.get('volume_utilization')))
        summary['per_unit_wt_cost'].append(sum(output_dict.get('cost'))/sum(output_dict.get('vehicle_load')))
        return summary
    ...
    def _get_output(output_dict, _orders, _products, config):
        ...
        for veh_no in output_dict.get('no_of_vehicles'):
            ...
            for idx, order_id in enumerate(output_dict.get('order_ids')[veh_no]):
                ...
                flatten_output['from_address'].append(_orders.loc[order_id, 'from_location'])
                flatten_output['to_address'].append(_orders.loc[order_id, 'to_location'])
                if config.vehicle_load_capacity_constraint:
    ...
    @staticmethod
    def _get_confusion_matrix(output_dict, vehicles, _orders, config, stat):
        ...
        for veh_no in output_dict.get('no_of_vehicles'):
            ...
            possible_vehicles = _vehicles[_vehicles['vehicle_type_name'] != v_typ].to_dict(orient='list')
    ...
    def solve(self):
        ...
        cm = self._get_confusion_matrix(self._solver_output, self._vehicles, self._orders, config, self.status)
        aggregated_output = self._get_aggregated_output(self._solver_output, self._orders, self.from_city, self.to_city)
        summary_output = self._get_summary(self._solver_output, self._summary, self.to_city)
