...
from route_planner.vrp.sku_variable_cost_planner.sku_variable_solver_configuration import SkuVariableSolverConfigurationV1
from route_planner.vrp.sku_variable_cost_planner.validator.base_validator import BaseValidator
from route_planner.vrp.sku_variable_cost_planner.validator.products_validator import ProductsValidator
from route_planner.vrp.sku_variable_cost_planner.validator.task_validator import TaskValidator
from route_planner.vrp.sku_variable_cost_planner.validator.vehicles_validator import VehiclesValidator
from solvers.solver_v1.exporter import export_routes_details, export_confusion_matrix, export_aggregated_details
...
class MultiProductVariableUploadValidation(object):
    ...
    def process_main(self):
        ...
        try:
            ...
            products_filter_bool = _products_df['sku'].isin(_task_df['sku'])
            products = _products_df[products_filter_bool]
            rid, temp_path = self.df_to_csv(products, self.products_validator)
            ...
            configuration = SkuVariableSolverConfigurationV1.get_default_sku_variable_configuration()
            planner = SolverV1(
            ...
            logger.info(f'cm: {cm}')
            logger.info(f'aggregated_output: {aggregated_output}')
    ...
    def df_to_csv(df, validator):
        rid, path = MultiProductVariableUploadValidation.get_path(validator.SHEET, validator.rid)
