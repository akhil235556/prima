...
from route_planner.vrp.sku_variable_cost_planner.validator import utils
...
class VehiclesValidator(BaseValidator):
    ...
    OUT_HEADER = ['vehicle_type_name', 'From', 'To', 'capacity', 'volumetric_capacity', 'SLA (hours)',
                  ...
                  'Driving window_To', 'dedicated', 'max_route_length', 'max_node_visits',
                  'No. of vehicles', 'from_city', 'to_city', 'daily_run']
    ...
    MANDATORY_COLUMNS = ['Vehicle Type*']
    ...
    def validate_charges(self):
        ...
        if not xor_bool:
            ...
            logger.info(f"Atmost one column should be provided {kg_col.name, km_col.name}")
            rows = list()
            ...
            logger.info(f"Value not provided for {km_col.name}")
            self.problems.extend(BaseValidator.add_problem(indexes, rows, message, self.SHEET))
        elif kg_bool.any():
    ...
    def atmost_one_validation(self):
        ...
        if not (km_bool ^ kg_bool).all():
            ...
            logger.info(f"Atmost one field should be provided ('Fixed charges', 'Per KM Charges', 'Per KG Charges')")
            self.problems.extend(BaseValidator.add_problem(indexes, rows, message, self.SHEET))
