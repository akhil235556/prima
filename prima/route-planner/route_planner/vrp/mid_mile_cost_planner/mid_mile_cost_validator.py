...
from route_planner.vrp.mid_mile_cost_planner.exporter import export_routes_details, export_aggregated_details, export_confusion_matrix
from route_planner.vrp.mid_mile_cost_planner.mid_mile_solver_configuration import MidMileSolverConfigurationV1
...
from route_planner.vrp.mid_mile_cost_planner.validator.task_validator import TaskValidator
from route_planner.vrp.mid_mile_cost_planner.validator.vehicles_validator import VehiclesValidator

from route_planner.constants.app_constants import UploadJobsStatus

logger = logging.getLogger(__name__)

class MidMileUploadValidation(object):
    ...
    def _valid_destination(self, value):
        if isinstance(value, str):
            vehicles_list = value.lower().split(',')
            return set(vehicles_list).issubset(set(self._VALID_DESTINATION_LIST))
        return False
    ...
    def destination_validation(self):
        """
        Validate destination col. in Vehicles by Valid locations
        """
        self._VALID_DESTINATION_LIST = self.task_validator.df['To Location*'].copy().str.lower().to_list()
        error = False
        col = self.vehicles_validator.df['Destination*'].dropna().copy()
        valid_rows = col.map(lambda x: self._valid_destination(x))
        print(valid_rows)
        if not valid_rows.all():
            indexes = valid_rows[valid_rows == False].index.to_list()
            filtered_rows = self.vehicles_validator.df.loc[indexes].copy()
            filtered_rows['Destination*'] = filtered_rows['Destination*'].map(
                lambda x: self._filter_error(x))
            rows = filtered_rows.to_dict(orient='records')
            message = f"Destination in Vehicle Destination not exists in Vehicle Sheet"
        ...
        if not error:
            ...
            col = self.vehicles_validator.new_df['destination'].copy().dropna()
            indexes = col.index.to_list()
            _VALID_VEHICLES = self.task_validator.df['To Location*'].copy().to_list()
            _valid_vehicle_map = {str(v).lower(): v for v in _VALID_VEHICLES}
            col = col.map(lambda x: MidMileUploadValidation._sanitize_serviceable_vehicles(x, _valid_vehicle_map))
            self.vehicles_validator.new_df['destination'].loc[indexes] = col.values
    ...
    def process_main(self):
        ...
        try:
            ...
            configuration = MidMileSolverConfigurationV1.get_default_mid_mile_configuration()
            planner = SolverV1(
    ...
    def df_to_csv(df, validator):
        rid, path = MidMileUploadValidation.get_path(validator.SHEET, validator.rid)
