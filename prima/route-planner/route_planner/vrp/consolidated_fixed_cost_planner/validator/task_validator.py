...
from route_planner.vrp.consolidated_fixed_cost_planner.validator.time_window_validator import TimeWindowValidator
...
class TaskValidator(BaseValidator):
    ...
    VALID_HEADER = ['Order ID*', 'From Location*', 'From Latitude*', 'From Longitude*',
                    ...
                    'To City*', 'Loading Time (Hours)', 'Unloading Time (Hours)', 'Load (kg)*',
                    'Volume (cbm)*', 'SLA (Hours)', 'Operational Hours From',
                    'Operational Hours To', 'Serviceable Vehicles']

    OUT_HEADER = ['order_id', 'from_location', 'from_latitude', 'from_longitude', 'to_location', 'to_latitude',
                  'to_longitude', 'load', 'volume', 'working_window_from', 'working_window_to',
                  'operating_window_from', 'operating_window_to', 'loading_time', 'unloading_time',
                  'serviceable_vehicles', 'from_city', 'to_city', 'task_id']
    ...
    MANDATORY_COLS = ['Order ID*', 'From Location*', 'From Latitude*', 'From Longitude*',
                      'To Location*', 'To Latitude*', 'To Longitude*', 'Load (kg)*', 'Volume (cbm)*',
                      'From City*', 'To City*']
    ...
    STRING_TYPE_COLS = ['From Location*', 'To Location*', 'From City*', 'To City*', 'Serviceable Vehicles']
    ...
    def sanitize(self):
        ...
        LOGGER.info("(Task sanitize) Cols After Mapping\n", self.new_df)
    ...
    def process(self):
        ...
        # add task_id
        self.new_df['task_id'] = self.df['Order ID*']
        # sanitize cols
        self.sanitize()
