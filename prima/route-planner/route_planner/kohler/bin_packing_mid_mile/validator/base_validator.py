...
class BaseValidator(object):
    ...
    def validate_sheets(self):
        ...
        if not set(BP_MID_MILE_SHEET_NAMES).issubset(set(wb.sheetnames)):
            raise InvalidRequest(message=f"Sheet not found!, Valid Sheets: {BP_MID_MILE_SHEET_NAMES}")
    ...
    @staticmethod
    def datetime_type_validator(self, col):

        cols1 = self.df[pd.to_datetime(self.df["Placement Date & Time*"], infer_datetime_format=True, errors='coerce').isna()]
    ...
    def df_to_csv(validator):
        rid = str(uuid.uuid4())
        temp_path = f"/tmp/{rid}_{validator.SHEET}.csv"
        validator.new_df.to_csv(temp_path, index=False)
        return rid, temp_path
