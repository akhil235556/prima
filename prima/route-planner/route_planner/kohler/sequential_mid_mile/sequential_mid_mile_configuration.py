...
class SequentialMidMileConfigDefault:
    # Boolean Constraints
    ...
    SKU_EXCLUSION_CONSTRAINT = False
...
class SequentialMidMileSolverConfigVariables:

    @property
    def vehicle_load_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_LOAD_CAPACITY_CONSTRAINT') or \
               SequentialMidMileConfigDefault.VEHICLE_LOAD_CAPACITY_CONSTRAINT

    @property
    def vehicle_volume_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_LOAD_CAPACITY_CONSTRAINT') or \
               SequentialMidMileConfigDefault.VEHICLE_VOLUME_CAPACITY_CONSTRAINT

    @property
    def vehicle_min_load_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_MIN_WEIGHT_CAPACITY_CONSTRAINT') or \
               SequentialMidMileConfigDefault.VEHICLE_MIN_LOAD_CAPACITY_CONSTRAINT

    @property
    def vehicle_min_load_capacity_per_ratio(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_MIN_WEIGHT_CAPACITY_PER_RATIO') or \
               SequentialMidMileConfigDefault.VEHICLE_MIN_LOAD_CAPACITY_PER_RATIO

    @property
    def vehicle_min_volume_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_MIN_VOLUME_CAPACITY_CONSTRAINT') or \
               SequentialMidMileConfigDefault.VEHICLE_MIN_VOLUME_CAPACITY_CONSTRAINT

    @property
    def vehicle_min_volume_capacity_per_ratio(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_MIN_VOLUME_CAPACITY_PER_RATIO') or \
               SequentialMidMileConfigDefault.VEHICLE_MIN_VOLUME_CAPACITY_PER_RATIO

    @property
    def sku_exclusion_constraint(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.SKU_EXCLUSION_CONSTRAINT') or \
               SequentialMidMileConfigDefault.SKU_EXCLUSION_CONSTRAINT

    @property
    def num_of_threads(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.NUM_OF_THREADS') or \
               SequentialMidMileConfigDefault.NUM_OF_THREADS

    @property
    def vehicle_repeat_factor(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.VEHICLE_REPEAT_FACTOR') or \
               SequentialMidMileConfigDefault.VEHICLE_REPEAT_FACTOR

    @property
    def integer_multiplier(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.INTEGER_MULTIPLIER') or \
               SequentialMidMileConfigDefault.INTEGER_MULTIPLIER

    @property
    def sequential_bool(self):
        return env.get('SOLVER_CONFIG.SEQUENTIAL_MID_MILE.SEQUENTIAL_BOOL') or \
               SequentialMidMileConfigDefault.SEQUENTIAL_BOOL


class SequentialMidMileSolverConfiguration(object):

    @staticmethod
    def get_default_sequential_mid_mile_configuration():
        config_vars = SequentialMidMileSolverConfigVariables()
