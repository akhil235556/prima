...
from route_planner.bin_packing.sequential_planner.validator.base_validator import BaseValidator
from route_planner.bin_packing.sequential_planner.validator.products_validator import ProductsValidator
from route_planner.bin_packing.sequential_planner.validator.task_validator import TaskValidator
from route_planner.bin_packing.sequential_planner.validator.vehicles_validator import VehiclesValidator
from route_planner.bin_packing.sequential_planner.bin_packing_solver_configuration import SequentialSolverConfigurationV4
...
from solvers.solver_v4.exporter_tsp import export_routes_details, export_aggregated_details, export_confusion_matrix_details, export_summary_info, export_vehicle_info
...
class SequentialUploadValidation(object):
    ...
    def process_main(self):
        ...
        for idx, task_df in enumerate(task_df_list):
            ...
            try:
                ...
                vehicles_df = SequentialUploadValidation.filter_vehicle(_vehicles_df, from_city, to_city)
                ...
                configuration = SequentialSolverConfigurationV4.get_default_sequential_configuration()
                ...
                try:
                    ...
                    raise RequestTerminationException(str(e))
                logger.info("Bin packing solver starting")
                ...
                logger.info(f"MIP: summary_dict: {summary_dict}")
                ...
                for order in agg_output['order_ids']:
                    route_orders = orders[orders['order_id'].isin(order)]
                    orders_list.append(route_orders.to_dict('records'))

                for vehicle in agg_output['vehicle_type']:
                    vehicles['No. of vehicles'] = 1
                    route_vehicle = vehicles[vehicles['vehicle_type_name'].isin([vehicle])]
                    vehicle_list.append(route_vehicle.to_dict('records'))

                for orders in orders_list:
                    for order in orders:
                        order_data.append(order)

                for vehicle_types in vehicle_list:
                ...
                summary_info = {}
                for route in range(len(orders_list)):
                    orders_df = pd.DataFrame(orders_list[route])
                    vehicle_df = pd.DataFrame(vehicle_list[route])
                    logger.info("VRP solver starting")
                    tsp_configuration = TSPSolverConfiguration.get_default_tsp_configuration()
                    planner = SolverTSP(
                        orders=orders_df, vehicles=vehicle_df, config=tsp_configuration,
        ...
        sheet_path = self.get_sheet(aggregated_data_dict, output_dict, confusion_matrix_dict, summary_dict,
                                    vehicle_dict)
    ...
    def get_sheet(self, agg_output, output, cm, summary, vehicles):
        ...
        excel_file_path = self.get_bin_packing_excel_file_path(rid=self._rid)
    ...
    def df_to_csv(idx, df, validator):
        rid, path = SequentialUploadValidation.get_path(validator.SHEET, validator.rid, idx)
