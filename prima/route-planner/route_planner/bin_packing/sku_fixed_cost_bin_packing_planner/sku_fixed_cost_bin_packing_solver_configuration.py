...
class SKUFixedCostSolverConfigVariables:
    ...
    def vehicle_load_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_LOAD_CAPACITY_CONSTRAINT') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_LOAD_CAPACITY_CONSTRAINT
    ...
    def vehicle_min_load_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_MIN_WEIGHT_CAPACITY_CONSTRAINT') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_MIN_LOAD_CAPACITY_CONSTRAINT

    @property
    def vehicle_min_load_capacity_per_ratio(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_MIN_WEIGHT_CAPACITY_PER_RATIO') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_MIN_LOAD_CAPACITY_PER_RATIO
    ...
    def vehicle_min_volume_capacity_constraint(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_MIN_VOLUME_CAPACITY_CONSTRAINT') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_MIN_VOLUME_CAPACITY_CONSTRAINT

    @property
    def vehicle_min_volume_capacity_per_ratio(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_MIN_VOLUME_CAPACITY_PER_RATIO') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_MIN_VOLUME_CAPACITY_PER_RATIO

    @property
    def sku_exclusion_constraint(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.SKU_EXCLUSION_CONSTRAINT') or \
               SKUFixedCostSolverConfigDefault.SKU_EXCLUSION_CONSTRAINT

    @property
    def num_of_threads(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.NUM_OF_THREADS') or \
               SKUFixedCostSolverConfigDefault.NUM_OF_THREADS

    @property
    def vehicle_repeat_factor(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.VEHICLE_REPEAT_FACTOR') or \
               SKUFixedCostSolverConfigDefault.VEHICLE_REPEAT_FACTOR

    @property
    def integer_multiplier(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.INTEGER_MULTIPLIER') or \
               SKUFixedCostSolverConfigDefault.INTEGER_MULTIPLIER

    @property
    def sequential_bool(self):
        return env.get('SOLVER_CONFIG.SKU_FIXED_COST_BIN_PACKING.SEQUENTIAL_BOOL') or \
               SKUFixedCostSolverConfigDefault.SEQUENTIAL_BOOL


class SkuFixedSolverConfigurationV4(object):
